    ...
    <div className="flex h-full flex-col pt-8 text-[#162019]">
      <div className="flex items-center justify-between px-3 py-2">
        <div className="flex items-center gap-1.5">
          ...
          <div>
            <p className="text-[5px] font-semibold uppercase tracking-wider text-[#7a8780]">
              Current
            </p>
      ...
      <div className="px-3">
        <div className="flex h-8 items-center gap-2 rounded-lg border border-[#dce2de] bg-white px-2.5">
          ...
          <span className="flex-1 text-[6px] text-[#66736c]">
            Search gyms or cities...
          </span>
        ...
        <div className="mt-2 flex gap-1.5">
          {["All", "Open Now", "Day Pass"].map((label, index) => (
            <span
              key={label}
              className={`rounded-full border px-2 py-1 text-[5.5px] font-medium ${
                index === 0
                  ? "border-[#173426] bg-[#173426] text-white"
                  : "border-[#d7ddd9] bg-white"
              }`}
            >
              {label}
            </span>
        ...
        <div className="mt-3 rounded-xl bg-[linear-gradient(135deg,#2d493c,#173426)] p-3 text-white shadow-[0_8px_18px_rgba(20,52,37,.18)]">
          <div className="flex items-center justify-between gap-2">
            <span className="rounded-sm bg-[#c8eed2] px-1.5 py-1 text-[5.5px] font-bold uppercase text-[#173426]">
              Active Pass
            </span>
            <span className="text-[5px] text-white/65">4h left</span>
          </div>
          <p className="mt-2 font-display text-[10px] font-semibold">
            Riverside Athletic Club
          </p>
          <p className="mt-1 text-[6px] text-white/70">Full club & spa access</p>
          <span className="mt-2 inline-flex rounded-md bg-[#c8eed2] px-2 py-1.5 text-[6px] font-semibold text-[#173426]">
            Show Pass
          </span>
        </div>
        ...
        <div className="overflow-hidden rounded-xl border border-[#dce1de] bg-white shadow-sm">
          <div className="relative h-[95px] overflow-hidden">
            ...
            <span className="absolute left-2 top-2 rounded bg-[#20372c]/90 px-2 py-1 text-[5px] text-white">
              ★ 4.8
            </span>
          ...
          <div className="p-2.5">
            <div className="flex justify-between gap-2">
              <p className="font-display text-[8px] font-semibold">
                Riverside Athletic Club
              </p>
              <p className="text-[7px] font-bold">
                £20 <span className="font-normal text-[#6f7f76]">/ day</span>
              </p>
            </div>
            <div className="mt-2 flex gap-1">
              {["Pool", "Sauna", "Weights"].map((tag) => (
                <span
                  key={tag}
                  className="rounded bg-[#edf4ef] px-1.5 py-1 text-[5px] text-[#47634f]"
                >
                  {tag}
                </span>
              ))}
  ...
  <PhoneFrame className="h-[400px] w-[195px] rotate-[7deg] [transform:rotateY(-10deg)_rotateZ(7deg)]">
    <div className="flex h-full flex-col pt-7 text-[#162019]">
      <div className="relative h-[150px] overflow-hidden">
        <img src={heroGym} alt="" className="h-full w-full object-cover" />
        <span className="absolute bottom-2 left-2 rounded bg-[#173426]/90 px-2 py-1 text-[5.5px] text-white">
          0.8 mi · London
        </span>
      </div>
      <div className="p-3">
        <div className="flex items-start justify-between gap-2">
          <div>
            ...
            <p className="mt-1 flex items-center gap-1 text-[5.5px] text-[#708078]">
              <MapPin className="h-2.5 w-2.5" /> Central London
            </p>
          </div>
          <p className="text-[8px] font-bold">
            £20<span className="text-[5px] font-normal"> / day</span>
          </p>
        </div>
        <p className="mt-3 text-[6px] leading-relaxed text-[#66736c]">
          Premium training space with flexible access and recovery facilities.
        </p>
        <div className="mt-3 grid grid-cols-3 gap-1">
          {["Pool", "Sauna", "Showers"].map((tag) => (
            <span
              key={tag}
              className="rounded-md bg-[#edf4ef] px-1 py-1.5 text-center text-[5px] text-[#47634f]"
            >
              {tag}
            </span>
          ))}
        </div>
        <span className="mt-4 flex w-full items-center justify-center rounded-lg bg-[#173426] py-2.5 text-[7px] font-semibold text-white">
          Buy Day Pass →
        </span>
        <div className="mt-4 rounded-lg border border-[#dde2df] bg-white p-2.5">
          <p className="text-[6px] font-semibold">Flexible access</p>
          <p className="mt-1 text-[5.5px] leading-relaxed text-[#708078]">
            No long-term membership. Clear entry details before you arrive.
          </p>
        </div>
      </div>
    </div>
...
const ProductPreview = ({ compact = false }: { compact?: boolean }) => (
  <div
    ...
    className={`pointer-events-none relative mx-auto select-none ${
      compact ? "h-[330px] w-[310px]" : "h-[470px] w-full max-w-[470px]"
    }`}
    style={{ perspective: "1200px" }}
  >
    <div
      className={`absolute left-1/2 top-1/2 -translate-x-[84%] -translate-y-1/2 ${
        compact ? "scale-[0.72]" : ""
      }`}
    >
      <DiscoverPhone />
    ...
    <div
      className={`absolute left-1/2 top-1/2 -translate-x-[18%] -translate-y-[46%] ${
        compact ? "scale-[0.72]" : ""
      }`}
    >
      <DetailPhone />
      ...
      <div className="relative overflow-hidden rounded-[2rem] border border-primary/10 bg-[linear-gradient(135deg,hsl(var(--primary))_0%,#294438_100%)] px-6 py-8 text-primary-foreground shadow-[0_26px_70px_rgba(20,36,27,0.16)] md:px-10 md:py-10 lg:grid lg:grid-cols-[1.08fr_0.72fr] lg:items-center lg:gap-10">
        <div className="relative z-10 max-w-2xl">
          ...
          <div className="mt-5 flex flex-wrap gap-x-5 gap-y-2 text-sm text-white/75">
            {["Discover gyms faster", "Compare facilities", "Buy flexible passes"].map(
              (item) => (
                <span key={item} className="flex items-center gap-2">
                  <Check className="h-4 w-4 text-[#c8eed2]" />
                  {item}
                </span>
              ),
            )}
          ...
          <p className="mt-3 text-xs text-white/55">
            Founding waitlist · Early access · No spam
          </p>
        </div>
  ...
  useEffect(() => {
    ...
    const bridgeCta = sections.find(
      (section) =>
        ...
        section.className.includes("bg-primary"),
    );
    ...
    const midTarget =
      contentSections[Math.max(0, Math.floor(contentSections.length * 0.55) - 1)];
    ...
    const heroPrimaryCta = hero.querySelector<HTMLAnchorElement>(
      'a[href="#waitlist"]',
    );
